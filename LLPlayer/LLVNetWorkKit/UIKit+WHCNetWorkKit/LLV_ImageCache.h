...
typedef void(^LLVImageQueryFinished)(UIImage * _Nullable image , UIControlState state);

/**
 * 说明: LLV_ImageCache 网络下载图片缓存类
 */

@interface LLV_ImageCache : NSObject

/**
 * 说明: LLV_ImageCache 单例对象
 */
...
- (void)queryImageForUrl:(nonnull NSString *)strUrl
             ...
             didFinished:(nullable LLVImageQueryFinished)finishedBlock;
@end
