...
#import "LLV_DataModel.h"
#import <objc/runtime.h>
#define kLLVKey    (@"")
@interface LLV_DataModel (){
    
}

@end
@implementation LLV_DataModel
...
+ (NSArray *)dataModelWithArrayData:(NSData *)data className:(Class)className{
    ...
    return [LLV_DataModel dataModelWithArray:array className:className];
}

+ (NSArray*)dataModelWithArray:(NSArray*)array className:(Class)className{
    NSMutableArray * _array = [NSMutableArray new];
    for (NSDictionary * dict in array) {
        [_array addObject:[LLV_DataModel dataModelWithDictionary:dict className:className]];
    }
    return _array;
}

+ (id)dataModelWithDictionary:(NSDictionary*)dictionary className:(Class)className{
    LLV_DataModel  * LLVDataModel = [LLV_DataModel new];
    id object = [LLVDataModel handleDataModelEngine:dictionary arrKey:kLLVKey calssName:className];
    return object;
}

+ (id)dataModelWithDictionaryData:(NSData *)data className:(Class)className{
    NSDictionary * dict = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingAllowFragments error:nil];
    return [LLV_DataModel dataModelWithDictionary:dict className:className];
}
