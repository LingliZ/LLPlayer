...
const NSInteger kLLVDefaultDownloadNumber = 3;

@interface LLV_HttpManager () {
    NSOperationQueue     * _httpOperationQueue;
    NSOperationQueue     * _fileDownloadOperationQueue;
    LLVReachability         * _internetReachability;
...
@implementation LLV_HttpManager

+ (nonnull instancetype)shared {
    static  LLV_HttpManager * LLVHttpManager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        LLVHttpManager = [LLV_HttpManager new];
    });
    return LLVHttpManager;
}
...
- (void)registerNetworkStatusMoniterEvent {
    ...
    _internetReachability = [LLVReachability reachabilityForInternetConnection];
    [_internetReachability startNotifier];
    [self updateInterfaceWithReachability:_internetReachability];
}

- (void)updateInterfaceWithReachability:(LLVReachability*)internetReachability{
    NetworkStatus netStatus = [internetReachability currentReachabilityStatus];
    self.networkStatus = netStatus;
    switch (netStatus) {
        case NotReachable:{
            for (LLV_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
                [downloadOperation cancelDownloadTaskAndDeleteFile:NO];
            }
            for (LLV_BaseOperation * httpOperation in _httpOperationQueue.operations) {
...
- (void)reachabilityChanged:(NSNotification *)notifiy{
    LLVReachability* curReach = [notifiy object];
    NSParameterAssert([curReach isKindOfClass:[LLVReachability class]]);
    [self updateInterfaceWithReachability:curReach];
}

#pragma mark - get请求 -

- (nullable LLV_HttpOperation *)get:(nonnull NSString *)strUrl
...
- (nullable LLV_HttpOperation *)get:(nonnull NSString *)strUrl
               ...
               didFinished:(nullable LLVDidFinished)finishedBlock {
    LLV_HttpOperation * getOperation = nil;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        getOperation = [LLV_HttpOperation new];
        getOperation.requestType = LLVHttpRequestGet;
        getOperation.progressBlock = processBlock;
        getOperation.strUrl = strUrl;
        __weak typeof(self) weakSelf = self;
        getOperation.didFinishedBlock = ^(LLV_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
...
- (nullable LLV_HttpOperation *)post:(nonnull NSString *)strUrl
...
- (nullable LLV_HttpOperation *)post:(nonnull NSString *)strUrl
                    ...
                    process:(nullable LLVProgress) processBlock
                didFinished:(nullable LLVDidFinished)finishedBlock {
    LLV_HttpOperation * postOperation = nil ;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        postOperation = [LLV_HttpOperation new];
        postOperation.requestType = LLVHttpRequestPost;
        postOperation.progressBlock = processBlock;
        postOperation.postParam = param;
        postOperation.strUrl = strUrl;
        __weak typeof(self) weakSelf = self;
        postOperation.didFinishedBlock = ^(LLV_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
            if (!isSuccess && error.code == 404) {
                [weakSelf.failedUrls addObject:strUrl];
            }
            if (finishedBlock) {
...
- (nullable LLV_HttpOperation *)upload:(nonnull NSString *)strUrl
...
- (nullable LLV_HttpOperation *)upload:(nonnull NSString *)strUrl
    ...
    LLV_HttpOperation * uploadOperation = nil ;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        uploadOperation = [LLV_HttpOperation new];
        [self setHttpOperation:uploadOperation];
        uploadOperation.requestType = LLVHttpRequestFileUpload;
        uploadOperation.progressBlock = processBlock;
        uploadOperation.strUrl = strUrl;
        uploadOperation.postParam = _uploadPostData;
        __weak typeof(self) weakSelf = self;
        uploadOperation.didFinishedBlock = ^(LLV_BaseOperation *operation, NSData *data, NSError *error, BOOL isSuccess) {
        ...
        uploadOperation.contentType = [NSString stringWithFormat:@"multipart/form-data; charset=%@; boundary=%@", charset, kLLVUploadCode];
...
- (nullable LLV_DownloadOperation *)download:(nonnull NSString *)strUrl
...
- (nullable LLV_DownloadOperation *)download:(nonnull NSString *)strUrl
    ...
    LLV_DownloadOperation  * downloadOperation = nil;
    NSString * fileName = nil;
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        fileName = [self handleFileName:saveFileName url:strUrl];
        for (LLV_DownloadOperation * tempDownloadOperation in _fileDownloadOperationQueue.operations) {
            if ([fileName isEqualToString:tempDownloadOperation.saveFileName]){
                ...
                if (delegate && [delegate respondsToSelector:@selector(LLVDownloadResponse:error:ok:)]) {
                    [delegate LLVDownloadResponse:tempDownloadOperation error:error ok:NO];
                } else if (delegate && [delegate respondsToSelector:@selector(LLVDownloadDidFinished:data:error:success:)]) {
                    [delegate LLVDownloadDidFinished:tempDownloadOperation data:nil error:error success:NO];
                }
                return tempDownloadOperation;
            }
        }
        if([self createFileSavePath:savePath]) {
            downloadOperation = [LLV_DownloadOperation new];
            downloadOperation.requestType = LLVHttpRequestGet;
        ...
        if (delegate &&
            ...
            [delegate LLVDownloadDidFinished:downloadOperation data:nil error:error success:NO];
        }
    }
...
NSLog(@"_fileDownloadOperationQueue.operations = %d",_fileDownloadOperationQueue.operations.count);
    ...
    if (strUrl != nil && ![_failedUrls containsObject:strUrl]) {
        ...
        if([self createFileSavePath:savePath]) {
            ...
            downloadOperation.strUrl = strUrl;
            __weak typeof(self) weakSelf = self;
            downloadOperation.didFinishedBlock = ^(LLV_BaseOperation *operation,
                                                   NSData *data,
                                                   NSError *error,
                                                   BOOL isSuccess) {
                if (!isSuccess && error.code == 404) {
...
- (BOOL)waitingDownload {
    return _fileDownloadOperationQueue.operations.count > kLLVDefaultDownloadNumber;
}
...
- (nullable NSString *)handleFileName:(NSString *)saveFileName url:(NSString *)strUrl {
    if (!_fileDownloadOperationQueue) {
        ...
        _fileDownloadOperationQueue.maxConcurrentOperationCount = kLLVDefaultDownloadNumber;
    }
...
//返回指定文件名下载对象
- (nullable LLV_DownloadOperation *)downloadOperationWithFileName:(nonnull NSString *)fileName {
    LLV_DownloadOperation * downloadOperation = nil;
    for (LLV_DownloadOperation * tempDownloadOperation in _fileDownloadOperationQueue.operations) {
...
- (void)cancelAllDownloadTaskAndDelFile:(BOOL)isDelete {
    for (LLV_DownloadOperation * operation in _fileDownloadOperationQueue.operations) {
...
- (LLV_DownloadOperation *)replaceCurrentDownloadOperationBlockResponse:(nullable LLVResponse)responseBlock
                                             process:(nullable LLVProgress)processBlock
...
- (LLV_DownloadOperation *)replaceCurrentDownloadOperationDelegate:(nullable id<LLV_DownloadDelegate>)delegate
                                       fileName:(nonnull NSString *)fileName {
    for (LLV_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
        if([downloadOperation.saveFileName isEqualToString:fileName]){
...
//替换所有当前下载代理
- (LLV_DownloadOperation *)replaceAllDownloadOperationBlockResponse:(nullable LLVResponse)responseBlock
                                         process:(nullable LLVProgress)processBlock
                                     didFinished:(nullable LLVDidFinished)didFinishedBlock {
...
- (LLV_DownloadOperation *)replaceAllDownloadOperationDelegate:(nullable id<LLV_DownloadDelegate>)delegate {
    if (_fileDownloadOperationQueue.operations.count > 0) {
        for (LLV_DownloadOperation * downloadOperation in _fileDownloadOperationQueue.operations) {
            downloadOperation.progressBlock = nil;
            downloadOperation.responseBlock = nil;
...
- (void)cancelHttpRequestWithUrl:(nonnull NSString *)url {
    for (LLV_BaseOperation * operation in _httpOperationQueue.operations) {
...
- (__autoreleasing NSError *)error:(nonnull NSString *)message {
    __autoreleasing NSError  * error = [[NSError alloc]initWithDomain:kLLVDomain
...
- (void)setHttpOperation:(LLV_BaseOperation *)httpOperation {
...
- (void)buildMultipartFormDataPostBody {
    ...
    NSString *stringBoundary = kLLVUploadCode;
