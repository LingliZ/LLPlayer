...
@interface LLV_DownloadOperation () {
...
@implementation LLV_DownloadOperation
...
- (void)start {
    ...
    if(![fm fileExistsAtPath:self.saveFilePath]) {
        ...
        NSString  * strRange = [NSString stringWithFormat:kLLVRequestRange ,_localFileSizeLenght];
    ...
    if(error == nil) {
        ...
        NSLog(@"%@",kLLVCalculateFolderSpaceAvailableFailError);
    }
    [super start];
    [self startRequest];
}
...
- (void)cancelDownloadTaskAndDeleteFile:(BOOL)isDelete {
    ...
    NSError * error = nil;
    if (!isDelete) {
        error = [NSError errorWithDomain:kLLVDomain
                            code:[self getCode]
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.didFinishedBlock) {
            ...
            [self.delegate LLVDownloadDidFinished:self data:nil error:error success:NO];
        }
    });
...
- (void)handleReqeustError:(NSError *)error code:(NSInteger)code {
    if (code != LLVCancelDownloadError) {
        [self removeDownloadFile];
    }
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response {
    ...
    if (![self handleResponseError:response]){
        ...
        if([self calculateFreeDiskSpace] < _actualFileSizeLenght){
            error = [[NSError alloc]initWithDomain:kLLVDomain
                                              code:LLVFreeDiskSpaceLack
                                          userInfo:@{NSLocalizedDescriptionKey:[NSString stringWithFormat:kLLVFreeDiskSapceError,_actualFileSizeLenght]}];
            [self removeDownloadFile];
            code = LLVFreeDiskSpaceLack;
            isCancel = YES;
            goto LLV1;
    ...
    LLV1:
        ...
        error = [NSError errorWithDomain:kLLVDomain code:code userInfo:@{NSLocalizedDescriptionKey:response.description}];
        ...
        dispatch_async(dispatch_get_main_queue() , ^{
            if (self.responseBlock) {
                ...
                self.responseBlock = nil;
                      ...
                      [self.delegate respondsToSelector:@selector(LLVDownloadResponse:error:ok:)]) {
                [self.delegate LLVDownloadResponse:self error:error ok:!isCancel];
            }
        });
    }
}
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    ...
    if(self.responseData.length > kLLVWriteSizeLenght && _fileHandle){
        [_fileHandle writeData:self.responseData];
        [self clearResponseData];
    }
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.progressBlock) {
            self.progressBlock(self ,self.recvDataLenght , _actualFileSizeLenght , self.networkSpeed);
                  ...
                  [self.delegate respondsToSelector:@selector(LLVDownloadProgress:recv:total:speed:)]) {
            [self.delegate LLVDownloadProgress:self recv:self.recvDataLenght total:_actualFileSizeLenght speed:self.networkSpeed];
        }
    });
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if (self.didFinishedBlock) {
            ...
            self.didFinishedBlock = nil;
                  ...
                  [self.delegate respondsToSelector:@selector(LLVDownloadDidFinished:data:error:success:)]) {
            [self.delegate LLVDownloadDidFinished:self data:nil error:nil success:YES];
        }
    });
