...
@implementation LLVGIFImageView
...
+ (NSMutableArray*)getGifFrames:(NSData*)gifImageData {
	LLVGIFImageView* gifImageView = [[LLVGIFImageView alloc] initWithGIFData:gifImageData];
	if (!gifImageView) {
		return nil;
	}
...
- (id)initWithGIFData:(NSData*)gifImageData {
	...
	if (![LLVGIFImageView isGifImage:gifImageData]) {
...
- (void)dealloc {
    ...
    if (GIF_buffer != nil)
    {
        GIF_buffer = nil;//[GIF_buffer release];
    }
    
    if (GIF_screen != nil)
    {
        GIF_screen = nil;//[GIF_screen release];
    }
    
    if (GIF_global != nil)
    {
        GIF_global = nil;//[GIF_global release];
    }
    
    [GIF_frames removeAllObjects];GIF_frames = nil;//[GIF_frames release];
}
...
- (bool) GIFGetBytes:(int)length {
    ...
    {
        GIF_buffer = (NSMutableData *)[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)];//(NSMutableData *)[[GIF_pointer subdataWithRange:NSMakeRange(dataPointer, length)] retain];
        dataPointer += length;
		return YES;
	}
    else
