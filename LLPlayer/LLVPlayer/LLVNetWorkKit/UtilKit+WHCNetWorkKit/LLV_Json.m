...
+ (NSString*)jsonWithDictionary:(NSDictionary *)dictionary{
    assert(dictionary);
    LLV_Json  * LLVJson = [LLV_Json new];
    return [NSString stringWithFormat:@"%@}",[LLVJson handleDictionaryEngine:dictionary]];
}
...
+ (NSDictionary *)dictionaryWithJson:(NSString *)json {
    assert(json);
    return [LLV_Json dictionaryWithJsonData:[json dataUsingEncoding:NSUTF8StringEncoding]];
}
...
+ (NSArray *)arrayWithJson:(NSString *)json {
    assert(json);
    return [LLV_Json arrayWithJsonData:[json dataUsingEncoding:NSUTF8StringEncoding]];
}
