...
+ (NSString*)xmlWithDictionary:(NSDictionary*)dictionary encode:(NSString*)encode rootAttribute:(NSString*)rootAttribute{
    LLV_Xml  * LLVXml = [[LLV_Xml alloc]initWithEncode:encode attribute:rootAttribute];
    return [LLVXml handleDictionaryEngine:dictionary arrKey:@"" root:YES];
}

+ (NSString*)xmlWithDictionary:(NSDictionary*)dictionary encode:(NSString*)encode{
    return [LLV_Xml xmlWithDictionary:dictionary encode:encode rootAttribute:@""];
}

+ (NSString*)xmlWithDictionary:(NSDictionary*)dictionary{
    return [LLV_Xml xmlWithDictionary:dictionary encode:@"utf-8"];
}

+ (NSString*)xmlWithDictionary:(NSDictionary*)dictionary rootAttribute:(NSString*)rootAttribute{
    return [LLV_Xml xmlWithDictionary:dictionary encode:@"utf-8" rootAttribute:rootAttribute];
}
